...
SOURCE="$ROOT/specs/003-accept-terms/contracts/terms-access-internal-v1.openapi.yaml"
OUTPUT="$ROOT/services/ingestion/src/clients/generated/terms-access.ts"
...
if [[ "${1:-}" == "--check" ]]; then
  ...
  if ! cmp -s "$temporary" "$OUTPUT"; then
    echo "ERROR: generated terms internal API types are stale; run scripts/contracts/generate-terms-internal.sh" >&2
    diff -u "$OUTPUT" "$temporary" || true
    exit 1
  fi
  echo "Tipos internos de acceso terms: PASS"
else
