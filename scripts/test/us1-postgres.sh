...
for _ in $(seq 1 60); do
  if podman exec "$NAME" pg_isready -U postgres -d alternative_scoring >/dev/null 2>&1; then break; fi
  sleep .5
done
...
TEST_DATABASE_URL="postgresql://postgres:integration-password@127.0.0.1:${PORT}/alternative_scoring" \
  ...
  tests/integration/applications/application-lifecycle.test.ts \
  tests/integration/evaluations/evaluate-application.test.ts \
  tests/authorization/us1_application_scope.test.ts

echo "US1 PostgreSQL applications, evaluations and authorization: PASS"
