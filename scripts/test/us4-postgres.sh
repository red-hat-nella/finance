...
for _ in $(seq 1 60); do
  podman exec "$NAME" pg_isready -U postgres -d alternative_scoring >/dev/null 2>&1 && break
  sleep .5
done
...
TEST_DATABASE_URL="postgresql://postgres:integration-password@127.0.0.1:${PORT}/alternative_scoring" \
  ...
  tests/integration/audit/audit-integrity.test.ts \
  tests/authorization/us4_audit_scope.test.ts
echo "US4 PostgreSQL append-only audit and authorization: PASS"
